...
(function($) {
    $.fn.showcase = function (options) {
        ...
        if (opt.images.length != 0) {
            ...
            for (var i in opt.images) {
                ...
                img.btn = opt.images[i].btn;
		img.select = opt.images[i].select;
                img.alt = opt.images[i].description || "";
		var imgBtn = $("<img />").attr({ "src": opt.images[i].btn, "border": "0" });
    ...
    $.fn.showcase.start = function($container, opt) {
	    ...
	    //$navBtn = $("<img />").attr({ "src": opt.images[i].btn, "border": "0" }).appendTo($navElement);
            ...
            if (opt.navigator.showMiniature) {
                $("<img />").attr({ src: $(this).find("img").attr("btn"), width: $navElement.css("width").replace("px", ""), height: $navElement.css("height").replace("px", ""), border: "0px" }).appendTo($navElement);
		$("<div />").css(opt.navigator.select.css).appendTo($navElement);
		//$("<div />").attr({ "class": "select" }).appendTo($navElement);
	    }
        });
	...
	$.fn.showcase.setNavigationItem = function(i, $container, opt) {
	    ...
	    $container.find("#navigator a").eq(i).addClass(opt.navigator.item.cssClassSelected).data("selected", true);
			...
			if (opt.navigator.item.cssClass) {
				...
				$container.find("#navigator a").eq(i).css(opt.navigator.item.cssSelected,opt.navigator.select.cssSelected).data("selected", true);
				//$container.find("#navigator a").eq(i).css(opt.navigator.select.cssSelected).data("selected", true);
			}
			else {
	...
	navSelect: { css: { border: "6px solid #fff" } },
		...
		navigator: {
			...
			cssHover: {
			    background: "url(images/theme/flash/select.png) no-repeat"
			    },
			cssSelected: {
			    background: "url(images/theme/flash/select.png) no-repeat"
			    }
